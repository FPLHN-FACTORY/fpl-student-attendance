...
const fetchDataAllStats = () => {
  loadingStore.show()
  requestAPI
    .get(`${API_ROUTES_STAFF.FETCH_DATA_STATISTICS}/${dataFilter.semester}`)
    .then(({ data: response }) => {
...
const fetchDataList = () => {
  ...
  requestAPI
    .get(`${API_ROUTES_STAFF.FETCH_DATA_STATISTICS}/${dataFilter.semester}/list-factory`, {
      params: {
...
const fetchDataUser = () => {
  ...
  requestAPI
    .get(`${API_ROUTES_STAFF.FETCH_DATA_STATISTICS}/${dataFilter.semester}/list-user`)
    .then(({ data: response }) => {
...
const fetchSendMail = () => {
  ...
  requestAPI
    .post(`${API_ROUTES_STAFF.FETCH_DATA_STATISTICS}/${dataFilter.semester}`, {
      ...formData,
...
watch(
  dataStats,
  (data) => {
    stats.value[0].value = data.totalProject || 0
    stats.value[1].value = data.totalFactory || 0
    stats.value[2].value = data.totalTeacher || 0
    stats.value[3].value = data.totalStudent || 0

    plan_stats.value[0].value = data.totalPlan || 0
    plan_stats.value[1].value = data.totalPLanProcess || 0
    plan_stats.value[2].value = data.totalPLanComplete || 0
    plan_stats.value[3].value = data.totalPLanCancel || 0
...
<template>
  ...
  >
    ...
    >
      ...
      <a-form-item class="col-sm-12" label="Gửi tới Giảng viên (tuỳ chọn):">
        <a-select
          class="w-100"
          placeholder="Chọn 1 hoặc nhiều Giảng viên muốn nhận báo cáo"
          v-model:value="formData.emailTeacher"
          :disabled="modalSendMail.isLoading"
          mode="multiple"
          allow-clear
        >
          <a-select-option v-for="o in lstUser.teacher" :key="o.id" :value="o.email">
            {{ o.code }} - {{ o.name }}
          </a-select-option>
        </a-select>
      </a-form-item>
      ...
      <div class="col-xl-4 col-lg-12">
        <a-card :bordered="false" class="dashboard-bar-chart">
          ...
          <div class="card-content mt-3">
            <p>Chi tiết số lượng kế hoạch dự án:</p>
          </div>
