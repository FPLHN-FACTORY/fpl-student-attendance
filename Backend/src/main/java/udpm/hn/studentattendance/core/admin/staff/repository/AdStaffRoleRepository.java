            ...
            value = """
                SELECT 
                       ...
                       CASE
                           WHEN r.id IN (
                                SELECT s.roles.size 
                                FROM UserStaff s
                                WHERE TRIM(s.id) LIKE CONCAT('%', TRIM(:#{#adStaffRoleRequest.staffId}), '%')
                                  AND s.status = 0
                           )
                           THEN 'true'
                           ELSE 'false'
                ...
                WHERE r.status = 0 
                  ...
                  AND (
                     ...
                     OR TRIM(:#{#adStaffRoleRequest.roleCode}) = ''
                     OR str(r.code) LIKE CONCAT('%', TRIM(:#{#adStaffRoleRequest.roleCode}), '%')
                  )
                  AND (
