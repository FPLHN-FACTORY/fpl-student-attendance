...
public interface STDStatisticsSemesterRepository extends SemesterRepository {
    ...
    @Query(value = """
            WITH factory_stats AS (
                ...
                LEFT JOIN attendance ad ON us.id = ad.id_user_student AND pd.id = ad.id_plan_date AND ad.status = 1            WHERE 
                    us.id = :idStudent AND
            ...
            SELECT
                ...
                COUNT(DISTINCT CASE 
                    WHEN end_date < UNIX_TIMESTAMP(NOW()) * 1000 AND absent_rate <= 0.2
                    THEN factory_id 
                END) AS pass,
                COUNT(DISTINCT CASE 
                    WHEN absent_rate > 0.2
                    THEN factory_id 
                END) AS fail,
                COUNT(DISTINCT CASE 
                    WHEN start_date <= UNIX_TIMESTAMP(NOW()) * 1000 
                    AND end_date >= UNIX_TIMESTAMP(NOW()) * 1000
                    AND absent_rate <= 0.2
                    THEN factory_id 
                END) AS process
            FROM factory_stats
