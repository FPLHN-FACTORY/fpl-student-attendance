...
import udpm.hn.studentattendance.core.admin.levelproject.model.response.LevelProjectResponse;
import udpm.hn.studentattendance.repositories.LevelProjectRepository;

@Repository
public interface AdminLevelProjectRepository extends LevelProjectRepository {
...
""", countQuery = """
        ...
        WHERE
            ...
            AND (:#{#request.status} IS NULL OR status = :#{#request.status})
""", nativeQuery = true)
    Page<LevelProjectResponse> getAll(Pageable pageable, AdminLevelProjectSearchRequest request);
}
