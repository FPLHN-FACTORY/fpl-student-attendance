...
public interface STAttendanceRecoveryHistoryLogRepository extends ImportLogRepository {
    ...
    @Query(value = """
            ...
            JOIN import_log il
            WHERE 
            ild.id_import_log = il.id AND 
            ild.status = 1 AND
            il.type = :type AND
            il.id_user = :userId AND
            (:facilityId IS NULL OR il.id_facility = :facilityId) AND
            ild.id_import_log = :idImportLog 
            """, nativeQuery = true)
    Integer getAllLine(String idImportLog, String userId, String facilityId, Integer type);

    ImportLog getImportLogById(String idImportLog);

    @Query(value = """
            SELECT ild
            FROM ImportLogDetail ild
            WHERE ild.importLog.id = :importLogId
            """)
    List<ImportLogDetail> getAllByImportLog(String importLogId);
}
