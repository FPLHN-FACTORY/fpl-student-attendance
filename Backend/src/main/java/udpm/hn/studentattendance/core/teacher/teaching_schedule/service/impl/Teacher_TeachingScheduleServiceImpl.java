...
public class Teacher_TeachingScheduleServiceImpl implements Teacher_TeachingScheduleService {
    ...
    @Override
    public ByteArrayInputStream exportTeachingSchedule(List<Teacher_TeachingScheduleResponse> teachingScheduleResponseList) {
        ...
        try {
            ...
            Stream.of("Ngày dạy", "Ca học", "Điểm danh muộn tối đa (phút)", "Mã môn", "Xưởng", "Dự án", "Mô tả")
                    .forEach(headerTitle -> {
            ...
            for (Teacher_TeachingScheduleResponse teachingScheduleResponse : teachingScheduleResponseList) {
                ...
                // Cột "Ngày dạy"
                String formattedTeachingDay = dateFormat.format(new Date(teachingScheduleResponse.getTeachingDay()));
                ...
                // Cột "Ca học"
                PdfPCell shiftCell = new PdfPCell(new Phrase(String.valueOf(teachingScheduleResponse.getShift()), cellFont));
                styleCell(shiftCell, backgroundColor);
                pdfTable.addCell(shiftCell);

                // Cột "Điểm danh muộn tối đa (phút)"
                PdfPCell lateArrivalCell = new PdfPCell(new Phrase(String.valueOf(teachingScheduleResponse.getLateArrival()), cellFont));
                styleCell(lateArrivalCell, backgroundColor);
                pdfTable.addCell(lateArrivalCell);

                // Cột "Mã môn"
                PdfPCell subjectCell = new PdfPCell(new Phrase(String.valueOf(teachingScheduleResponse.getSubjectCode()), cellFont));
                styleCell(subjectCell, backgroundColor);
                pdfTable.addCell(subjectCell);

                // Cột "Xưởng"
                PdfPCell factoryCell = new PdfPCell(new Phrase(String.valueOf(teachingScheduleResponse.getFactoryName()), cellFont));
                styleCell(factoryCell, backgroundColor);
                pdfTable.addCell(factoryCell);

                // Cột "Dự án"
                PdfPCell projectCell = new PdfPCell(new Phrase(String.valueOf(teachingScheduleResponse.getProjectName()), cellFont));
                styleCell(projectCell, backgroundColor);
                pdfTable.addCell(projectCell);

                // Cột "Mô tả"
                PdfPCell descriptionCell = new PdfPCell(new Phrase(
                        teachingScheduleResponse.getDescription() != null ? teachingScheduleResponse.getDescription() : "",
                        cellFont
                ));
