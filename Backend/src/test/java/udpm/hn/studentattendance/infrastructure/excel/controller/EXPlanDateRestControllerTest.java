...
class EXPlanDateRestControllerTest {
    @Mock
    private EXPlanDateService exPlanDateService;

    @InjectMocks
    private EXPlanDateRestController controller;
    ...
    void testGetDataFromFile() {
        ...
        when(exPlanDateService.getDataFromFile(request)).thenReturn((ResponseEntity) ResponseEntity.ok("data"));
    ...
    void testImportItem() {
        ...
        when(exPlanDateService.importItem(request)).thenReturn((ResponseEntity) ResponseEntity.ok("imported"));
    ...
    void testExportData() {
        ...
        when(exPlanDateService.exportData(request)).thenReturn((ResponseEntity) ResponseEntity.ok("exported"));
    ...
    void testDownloadTemplate() {
        EXDataRequest request = new EXDataRequest();
        when(exPlanDateService.downloadTemplate(request)).thenReturn((ResponseEntity) ResponseEntity.ok("template"));
    ...
    void testHistoryLog() {
        EXDataRequest request = new EXDataRequest();
        when(exPlanDateService.historyLog(request)).thenReturn((ResponseEntity) ResponseEntity.ok("history"));
    ...
    void testHistoryLogDetail() {
        EXDataRequest request = new EXDataRequest();
        when(exPlanDateService.historyLogDetail(request, "1")).thenReturn((ResponseEntity) ResponseEntity.ok("detail"));
