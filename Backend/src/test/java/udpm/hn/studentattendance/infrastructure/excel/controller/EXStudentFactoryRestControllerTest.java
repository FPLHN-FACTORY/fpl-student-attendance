...
class EXStudentFactoryRestControllerTest {
    @Mock
    private EXStudentFactoryService exStudentFactoryService;

    @InjectMocks
    private EXStudentFactoryRestController controller;
    ...
    void testGetDataFromFile() {
        ...
        when(exStudentFactoryService.getDataFromFile(request)).thenReturn((ResponseEntity) ResponseEntity.ok("data"));
    ...
    void testImportItem() {
        ...
        when(exStudentFactoryService.importItem(request)).thenReturn((ResponseEntity) ResponseEntity.ok("imported"));
    ...
    void testExportData() {
        ...
        when(exStudentFactoryService.exportData(request)).thenReturn((ResponseEntity) ResponseEntity.ok("exported"));
    ...
    void testDownloadTemplate() {
        EXDataRequest request = new EXDataRequest();
        when(exStudentFactoryService.downloadTemplate(request))
                .thenReturn((ResponseEntity) ResponseEntity.ok("template"));
    ...
    void testHistoryLog() {
        EXDataRequest request = new EXDataRequest();
        when(exStudentFactoryService.historyLog(request)).thenReturn((ResponseEntity) ResponseEntity.ok("history"));
    ...
    void testHistoryLogDetail() {
        EXDataRequest request = new EXDataRequest();
        when(exStudentFactoryService.historyLogDetail(request, "1"))
                .thenReturn((ResponseEntity) ResponseEntity.ok("detail"));
